...
[[nodiscard]] std::optional<InputFormat> infer_input_format(const std::filesystem::path& path);

class PlotSessionState {
public:
    [[nodiscard]] FileLoadResult load_file(const std::filesystem::path& path,
        std::optional<InputFormat> format = std::nullopt, NmeaParseOptions nmea_options = {});
    [[nodiscard]] bool add_loaded_file(LoadedFile file);

    [[nodiscard]] bool set_file_visible(std::size_t slot_number, bool visible);
    [[nodiscard]] bool move_file(std::size_t from_slot_number, std::size_t to_slot_number);
    [[nodiscard]] bool erase_file(std::size_t slot_number);
    [[nodiscard]] bool set_file_override_hz(std::size_t slot_number, std::optional<double> hz);

    [[nodiscard]] bool set_common_time_range(CommonTimeRange range);
    [[nodiscard]] bool use_intersection_time_range();
    [[nodiscard]] bool set_enu_reference_configuration(EnuReferenceConfiguration configuration);
    [[nodiscard]] bool set_reference_match_configuration(ReferenceMatchConfiguration configuration);
