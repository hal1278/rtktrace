...
[[nodiscard]] std::optional<RelativeSample> make_relative_sample(
    const NormalizedSample& reference, const NormalizedSample& comparison,
    const ReferenceMatch& match) noexcept
{
    ...
    const RelativeSample relative{
        ...
        .delta_ellipsoidal_height_m = comparison.llh.ellipsoidal_height_m
            - reference.llh.ellipsoidal_height_m,
    ...
    if (!std::isfinite(relative.delta_enu.east_m)
        || !std::isfinite(relative.delta_enu.north_m)
...
{
    return cache.dependencies.has_value()
    ...
    ReferenceMatchConfiguration configuration)
{
    const std::optional<std::vector<ReferenceMatch>> matches = match_reference_epochs(
        reference_samples, reference_range, comparison_samples, comparison_range,
        configuration);
    ...
    for (const ReferenceMatch& match : *matches) {
        const std::optional<RelativeSample> sample = make_relative_sample(
            reference_samples[match.reference_index],
...
{
    if (!time_index.range.has_value()
        || time_index.file_ranges.size() != files.size()
        || time_index.revision == 0 || !enu_cache.reference.has_value()
        || enu_cache.revision == 0 || configuration.maximum_time_difference_ns < 0) {
    ...
    if (files.size() > 1) {
        ...
        for (std::size_t slot = 2; slot <= files.size(); ++slot) {
            ...
            std::optional<std::vector<RelativeSample>> relative_samples =
                make_relative_samples(files[0].samples, *reference_range,
                    files[slot - 1].samples, *comparison_range, configuration);
