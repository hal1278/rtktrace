...
基準相対表示では、スロット1を基準ファイルとし、スロット2以降の各ファイルについて相対水平位置、相対位置の成分別時系列および基準相対距離時系列を表示する。
...
ファイルサイズが100 MiB以上である場合は、読み込み前に警告し、利用者が読み込みの続行または中止を選択できるものとする。
...
#### FR-REF-003 対応しないsampleおよびtolerance

比較sample以前の基準epochが存在しない場合は、その比較sampleを基準相対表示へ出力しない。

利用者は、基準epochとの最大時刻差によるtolerance checkを有効または無効へ切り替えられる。

tolerance checkが有効であり、比較sampleと対応する基準epochとの時刻差が利用者設定可能な最大時刻差を超える場合は、その比較sampleを基準相対表示へ出力しない。

tolerance checkが無効である場合は、比較sample以前の最新基準epochが存在する限り、時刻差にかかわらず対応付ける。基準ファイルの最終epochより後の比較sampleには、その最終epochを継続して使用する。

最大時刻差および有効・無効時の判定方法は、`data-specification.md`で定義する。
...
#### FR-REF-006 基準相対距離時系列

相対位置の成分別時系列に加えて、各比較sampleと対応する基準epochとの三次元基準相対距離を、比較sampleの時刻に対して表示する。

基準相対距離は各matched pairの空間距離であり、連続する比較sample間の移動距離、軌跡に沿った道のりまたは累積距離ではない。距離の定義および計算方法は、`data-specification.md`で定義する。
...
左右の矢印キーではEast軸方向だけを移動し、上下の矢印キーではNorth軸方向だけを移動する。1回のキー操作による移動量は、対象軸の現在の表示範囲の5%とする。
...
最小値または最大値上のホイール1段当たりの変更量は、対象軸の現在の表示範囲の1%とする。
パネル表示では描画領域長を表示のみとし、直接変更しない。
...
時系列の描画領域にもFR-AREA-003の最小寸法および最大寸法の制約を適用する。
各縦軸および共有時刻軸の最小値または最大値上のホイール1段当たりの変更量は、
対象軸の現在の表示範囲の1%とする。描画領域長は表示のみとする。
...
利用者は、読み込み済みの全ファイルへ共通して適用する処理対象時刻範囲を指定できる。実効時刻範囲はstartおよびendを含む閉区間とする。

startおよびendは個別に有効または無効へ切り替えられる。有効な境界には入力値を使用し、無効な境界には読み込み済み全ファイルのunionの対応する境界を使用する。

startおよびendの初期状態はともに無効とし、初期の実効時刻範囲をunionとする。

#### FR-TIME-002 時刻範囲dialog

共通時刻範囲の指定dialogには、startおよびendの入力欄を設け、各入力欄の左側にその境界の有効・無効を切り替えるcheckboxを配置する。

checkboxを有効にした場合は対応する入力値を実効境界として使用する。checkboxを無効にした場合は入力値を消去せず、対応するunion境界を実効境界として使用する。

dialogには`Intersection` buttonを設ける。`Intersection`を実行した場合は、intersectionのstartおよびendを各入力欄へ設定し、両checkboxを有効にする。`Union` buttonは設けない。

buttonおよび入力操作はdialog内の値だけを更新し、利用者が`OK`を実行するまで時刻範囲を適用しない。intersectionが存在しない場合は入力値およびcheckboxを変更せず、そのことを通知する。
...
表示名は、モード行を`Normal`および`Reference`、各行の表示種別を`Trajectory`、
`Time Series`および`Both`とする。
...
基準相対表示では、比較sampleと同時刻またはそれ以前に存在する最新の基準epochを使用する。該当する基準epochが存在しない場合は、その比較sampleを表示しない。

tolerance checkが有効であり、その基準epochとの時刻差が設定された最大時刻差を超える場合も、その比較sampleを表示しない。tolerance checkが無効である場合は時刻差を検査せず、使用可能な最新基準epochを継続して使用する。

tolerance checkの有効・無効および最大時刻差は利用者が設定可能とする。
...
共通時刻範囲を指定するdialogには、startおよびendの入力欄を設け、各入力欄の左側に対応するcheckboxを配置する。

checkboxが有効な場合は入力値を使用し、無効な場合は対応するunion境界を使用する。
